...
/* C Hash Table. (Written in and for C89 and beyond.)
    ...
    make it "convenient", not really a problem tho.
*/
...
int main(void) {
    hc_Table(int) table = {0};
    table.items = malloc(sizeof(int)*1024);
    table.keys = malloc(sizeof(hc_TableKey)*1024);
    table.capacity = 1024;

    hc_table_insert(&table, "hello", 10);
    printf("%i\n", hc_table_get(&table, "hello"));
