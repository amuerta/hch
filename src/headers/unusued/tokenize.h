...
EXAMPLE:
    ...
    `struct { void*, size_t }` such that `sizeof(String) == 16`
 */
...
hc_Token str_chop_token(tkn_String* src, hc_TokenEntry* token_table, size_t count);
hc_Token str_chop_token_opt(tkn_String* src, hc_TokenEntry* token_table, size_t count, int flags);
/*                                                                                     ^^^^^^^^^
...
hc_Token str_chop_token_or_string(tkn_String *src, 
...
/*************************|Helper functions|*************************/
hc_Token str_chop_token(tkn_String* src, hc_TokenEntry* token_table, size_t count) {
    return str_chop_token_opt(src, token_table, count, 0);
}
        ...
        const char* opening, const char* closing) 
{
    ...
    t = str_chop_token(src, table, count);
    ...
    if(tkn_match_cstring(t, opening)) {
        before = t;
        while(t = str_chop_token(src, table, count), tkn_is_valid(t)) {
